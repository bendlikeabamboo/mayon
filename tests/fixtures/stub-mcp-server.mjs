...
rl.on('line', async (line) => {
	...
	try {
		...
		if (msg.method === 'initialize') {
			write({ jsonrpc: '2.0', id: id, result: { protocolVersion: '2025-06-18', capabilities: { tools: {} }, serverInfo: { name: 'stub-mcp-server', version: '0.0.1' } } });
		} else if (msg.method === 'tools/list') {
			write({ jsonrpc: '2.0', id: id, result: { tools: TOOLS } });
		} else if (msg.method === 'tools/call') {
			const args = msg.params?.arguments ?? {};
			write({ jsonrpc: '2.0', id: id, result: { content: [{ type: 'text', text: JSON.stringify(args) }] } });
