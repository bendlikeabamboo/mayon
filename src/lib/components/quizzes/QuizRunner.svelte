...
{#if quizzesStore.current}
	<div class="mx-auto flex max-w-3xl flex-col gap-4 p-6">
		...
		{:else if !quizzesStore.allAnswered}
			...
			<ol class="space-y-4">
				{#each quizzesStore.questions as q, ord (q.id)}
					...
					<li id="quiz-q-{q.id}" class="space-y-2 rounded-lg border border-border bg-card p-4">
		...
		{:else}
			...
			<AttemptHistory />
		{/if}
		<DiagnosticsPanel quizId={quizzesStore.current.id} title="Mayon console — Quiz" />
	</div>
{/if}
