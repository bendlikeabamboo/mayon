<script lang="ts">
	...
	onMount(async () => {
		await diagnosticsStore.load(chatId);
	});
	...
	<SheetContent side="right" class="w-[480px] sm:w-[540px]">
		...
		<div class="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
			<div class="flex items-center justify-between">
				...
				<Button variant="outline" size="sm" onclick={() => void diagnosticsStore.clear(chatId)}>
					<Trash2 class="size-3" />
					Clear traces
				</Button>
			</div>
			...
			<section class="space-y-1">
				{#each [...diagnosticsStore.traces].sort((a, b) => b.createdAt - a.createdAt) as trace (trace.id)}
					...
					{@const isEmpty = parsed?.persisted?.empty === true}
					{@const errorMsg =
					...
					>
						...
						{:else if parsed?.iterations?.[0]?.finishReason}
							<span class="text-xs text-muted-foreground">
			...
			{#if selectedTrace && selectedParsed}
				<section class="space-y-4 border-t border-border pt-4">
					<h3 class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
						Turn Detail — #{turnNumbers[selectedTrace.id] ?? 0}
					</h3>
					...
					{#each selectedParsed.iterations ?? [] as iter, i (i)}
						<div class="space-y-3">
							{#if i > 0}
							...
							{#if iter.receivedText}
								<div class="space-y-1">
									<div class="flex items-center justify-between">
										<span class="text-xs font-semibold text-muted-foreground"> Received Text </span>
										<Button
											variant="ghost"
											size="sm"
											class="h-6 px-2 text-xs"
							...
							{#if iter.toolResults && iter.toolResults.length > 0}
								<div class="space-y-1">
									<div class="flex items-center justify-between">
										<span class="text-xs font-semibold text-muted-foreground"> Tool Results </span>
										<Button
											variant="ghost"
											size="sm"
											class="h-6 px-2 text-xs"
