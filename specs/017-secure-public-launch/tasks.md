...
- [ ] T013 Create the open-mode parity half of `server/src/auth-gate.test.ts`: with mode open, every existing endpoint behaves exactly as today (`/api/health` 200 public, `/api/db/query` reaches its handler, existing suites in `server/src/server.test.ts` still green) — the gate must be invisible while open
