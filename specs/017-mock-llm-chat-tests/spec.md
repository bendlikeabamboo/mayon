...
- **Amendment (2026-09-02, implementation phase):** the suite surfaced two product bugs that blocked the round trip itself; both were fixed as documented, minimal exceptions to the zero-product-diff constraint, each regression-tested by this suite per constitution II ("every bug fix ships with a regression test"): (1) `awaitDb()` threw instead of starting boot when a universal route load ran before layout init — a cold direct load of `/chat` (or a refresh on it) rendered a 500; (2) the chat page's per-chat draft restore clobbered a prompt typed while the chat was still loading. No test-support capability entered the product, and the zero-diff guarantee for the mock/onboarding/request/render capability (SC-005's intent) is intact.
- A further product edge case was exposed but deliberately NOT fixed here (recorded as a follow-up): `buildSourceMap` logs "failed to parse closing fence; falling back to element position" for multi-line `$$ … $$` display-math blocks, which desynchronizes expound alignment for content after the math. The fixture uses single-line `$$ … $$` (renders identically) and the suite covers alignment over prose; fixing the sourcemap for multi-line display math deserves its own spec'd change with unit tests.
